...
describe Git do
  describe '.last_tag' do
    example do
      ...
      expect(Git.last_tag).to eq('v1.0.5-alpha.1')
    end

    example do
      expect(Git).to receive(:run).and_return('v1.10.0 v1.8.0')
      expect(Git.last_tag).to eq('v1.10.0')
    end

    example do
      expect(Git).to receive(:run).and_return("v1.10.1-dev.0\nv1.10.0\nv1.8.0\n\n")
      expect(Git.last_tag).to eq('v1.10.1-dev.0')
    end
  end
